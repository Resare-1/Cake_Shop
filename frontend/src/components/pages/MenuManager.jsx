...
import { getMenus, addMenu, toggleMenuStatus, getMenuById, updateMenu,updateMenuIngredients  } from "../../api/menuApi";
...
const fetchIngredients = async () => {
  try {
    const data = await getAllIngredients(); // ใช้ฟังก์ชันนี้แทน getIngredients()
...
const filtered = allIngredientsOriginal
  .filter(i => i.IngredientName && i.IngredientName.toLowerCase().includes(value.toLowerCase()))
  .filter((v, i, a) => a.findIndex(t => t.IngredientID === v.IngredientID) === i)
  .filter(i => !selectedIngredients.find(s => s.IngredientID === i.IngredientID)); // ตัด ingredient ที่เลือกแล้ว
...
const handleEditMenu = async (menuID) => {
  try {
    ...
    setForm({
      ...
      Bake_Time: menuData.Bake_Time,
      Is_Custom: menuData.Is_Custom,
    });
    // แก้ไขให้ qty_required มีค่า default
    setSelectedIngredients(menuData.Ingredients.map(i => ({
      ...i,
...
const handleUpdateMenu = async (e) => {
  ...
  try {
    ...
    // 1️⃣ อัปเดท menu ข้อมูลหลัก
    await updateMenu(editingMenuId, updatedMenu);

    // 2️⃣ อัปเดท ingredient ของเมนู
    await updateMenuIngredients(editingMenuId, ingredientsToUpdate);

    // 3️⃣ อัปเดท menus state
    setMenus(menus.map(m =>
      m.MenuID === editingMenuId
        ? { ...m, ...updatedMenu, Ingredients: selectedIngredients } // ใช้ selectedIngredients เดิม
        : m
    ));
  ...
  <div className="p-6 space-y-6 max-w-7xl mx-auto">
    ...
    <Card>
      <CardContent className="space-y-3">
        <h3 className="text-xl font-semibold">{editingMenuId ? "Edit Menu" : "Add New Menu"}</h3>
        <form
